	...
	public void run() {
        ...
        for(int x = c1.getBlockX(); x < c2.getBlockX(); x++) {
            ...
            for(int z = c1.getBlockZ(); z < c2.getBlockZ(); z++) {
                int type = CraftMagicNumbers.getId(world.getType(x, y, z));
                for(int i = 0; i < 256; i++) {
                    type = CraftMagicNumbers.getId(world.getType(x, i, z));
                    if(type != 0) {
        ...
        Collections.sort(HungerGames.BLOCKS, new Comparator<Block>() {
			...
			public int compare(Block b1, Block b2) {
				int d1 = this.getDistance(b1), d2 = this.getDistance(b2);
				if(d1 > d2) {
					return -1;
				} else if(d2 > d1) {
					return 1;
			...
			private int getDistance(Block b1) {
				...
				return (int) Math.sqrt((l.getX() - spawn.getX()) * (l.getX() - spawn.getX()) + (l.getZ() - spawn.getZ()) * (l.getZ() - spawn.getZ()));
			}
        });
