    ...
    public Games onPluginEnable() {
        ...
        new BukkitRunnable() {
            ...
            public void run() {
                ...
                // last possible listener, will always be called last
                /*registerListener(new Listener() {
                    @EventHandler(priority = EventPriority.MONITOR, ignoreCancelled = false)
                    public void onEntityDamage(EntityDamageEvent event) {
                        for (RegisteredListener listener : EntityDamageEvent.getHandlerList().getRegisteredListeners()) {
                            if (!listener.getListener().getClass().equals(getClass())) {
                                for (Method method : listener.getListener().getClass().getMethods()) {
                                    EntityDamageEvent damageEvent = new EntityDamageEvent(event.getEntity(), event.getCause(), event.getDamage());
                                    if (method.getParameters().length == 1 && method.getParameters()[0].getType().equals(EntityDamageEvent.class)) {
                                        try {
                                            method.invoke(listener.getListener(), damageEvent);
                                            if (damageEvent.isCancelled()) {
                                                getLogger().info("The EntityDamageEvent was cancelled by " + listener.getListener().getClass().getName() + "#" + method.getName() + "(" + method.getParameters()[0].getName() + ")");
                                            }
